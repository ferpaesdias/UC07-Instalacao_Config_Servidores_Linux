# Gerenciar contas de usuários

Este guia mostra como criar usuários e grupos de usuários da **EmpresaTech** no domínio `empresatech.example`. 

--- 

## Criação da Estrutura da Empresa

Vamos criar os grupos e usuários conforme o organograma da **EmpresaTech**.

<br>

## 👥 Estrutura de Usuários e Grupos

Neste laboratório, vamos gerenciar a autenticação centralizada. Estes são os funcionários da nossa empresa fictícia:

1. Departamento Financeiro (`Financeiro`)

   * Ana Souza (`ana.souza`)
   * Bruno Alves (`bruno.alves`)
   * Carla Dias (`carla.dias`)
   * Julia Pereira (`julia.pereira`)

   <br>

2. Recursos Humanos (`RH`)

   * Daniel Rocha (`daniel.rocha`)
   * Elisa Martins (`elisa.martins`)
   * Fabio Costa (`fabio.costa`)
   * Igor Santos (`igor.santos`)

   <br>

3. Tecnologia / TI (`TI`)

   * Gabriel Lima (`gabriel.lima`)
   * Helena Silva (`helena.silva`)

   <br>

4. Diretoria (`Diretoria`)

   * Luis Divino (`luis.divino`)
---


### Criar Grupos

#### Criar um grupo 

O comando abaixo cria um único grupo

```bash
samba-tool group add "nome_grupo" --description="descricao"
```
* `samba-tool group add`: É o comando do Samba4 para adicionar grupos.
* `--description=`: Descrição do grupo.
<br>

#### Criar grupos em massa

Segue abaixo como criar vários grupos ao mesmo tempo.

1. Preparar o Arquivo de Entrada (CSV)
Primeiro, precisamos de uma lista. Vamos criar um arquivo simples onde cada linha contém o **Nome do Grupo** e uma **Descrição**, separados por um ponto e vírgula `;`.
 
   ```bash
   vim lista_grupos.csv
   ```

   <br>
   
   Cole o conteúdo abaixo (exemplo):

   ```csv
   Financeiro;Grupo do Departamento Financeiro
   RH;Recursos Humanos e Departamento Pessoal
   TI;Tecnologia da Informação
   Diretoria;Grupo dos diretores da empresa
   ```
<br>

2. O Script de Automação

   Agora vamos criar o script que lê essa lista e executa o comando.
   
   Crie o arquivo do script:

   ```bash
   vim criar_grupos.sh
   ```

   <br>
   
   Cole o seguinte código:

   ```txt
   #!/bin/bash
   
   # =======================================================
   # SCRIPT: CRIAR GRUPOS EM MASSA NO SAMBA4 AD
   # =======================================================
   
   # 1. Definição do arquivo de entrada
   ARQUIVO_LISTA="lista_grupos.csv"
   
   # Verifica se o arquivo existe
   if [ ! -f "$ARQUIVO_LISTA" ]; then
       echo "Erro: O arquivo $ARQUIVO_LISTA não foi encontrado."
       exit 1
   fi
   
   echo "--- Iniciando a criação de grupos no AD ---"
   
   # 2. Leitura do arquivo linha por linha
   # IFS=';' define que o ponto e vírgula separa as variaveis
   while IFS=';' read -r nome_grupo descricao || [ -n "$nome_grupo" ]; do
       
       # Ignora linhas vazias
       if [ -z "$nome_grupo" ]; then
           continue
       fi
   
       # 3. Executa o comando samba-tool
       # --description adiciona a descrição no AD
       echo "Criando grupo: $nome_grupo..."
       
       samba-tool group add "$nome_grupo" --description="$descricao"
       
       # Verifica se deu certo (0 = sucesso)
       if [ $? -eq 0 ]; then
           echo "OK: Grupo '$nome_grupo' criado com sucesso."
       else
           echo "AVISO: Falha ao criar '$nome_grupo' (Talvez já exista?)"
       fi
       echo "-------------------------------------------"
   
   done < "$ARQUIVO_LISTA"
   
   echo "Importação finalizada."
   ```
   * `IFS=';'`: (Internal Field Separator) Diz ao comando `read` que, quando encontrar um ponto e vírgula na linha, ele deve separar o texto. A primeira parte vai para a variável `$nome_grupo` e a segunda para `$descricao`.
   * `|| [ -n "$nome_grupo" ]`: Um truque para garantir que o script leia até a última linha do arquivo de texto, mesmo que ela não termine com uma quebra de linha (Enter).

   <br>

3. Executando o Script

   Agora vamos dar permissão de execução ao script:
   
   ```bash
   chmod +x criar_grupos.sh
   ```

   <br>
   
   Execute o script:

   ```bash
   bash criar_grupos.sh
   ```
   <br>

4. Liste os grupos

   Liste os grupos para verificar ser foram criados corretamente:

   ```bash
   samba-tool group list
   ```
   <br>

5. Delete um grupo

   ```bash
   samba-tool delete nome_grupo
   ```
---

### Criar Usuários

#### Criar um usuário 

O comando abaixo cria um único grupo

```bash
samba-tool user create "nome.usuario" "SenhaForte123!" --must-change-at-next-login --use-username-as-cn
```
* `samba-tool user create`: É o comando do Samba4 para adicionar usuários.
* `--must-change-at-next-login`: Obriga o usuário alterar a senha no primeiro login.
* `--use-username-as-cn`: Garante que o "Nome de Exibição" no AD seja igual ao login, evitando erros se você não forneceu Nome e Sobrenome separados.

<br>

Adicione o usuário criado a um dos grupos adicionados anteriormente.

```bash
samba-tool group addmembers "nome_grupo" "nome.usuario" 
```

<br>

#### Criar usuários em massa

Segue abaixo como criar vários usuarios e adicioná-los ao seu grupo ao mesmo tempo.

>Consideramos que os grupos já foram criados.

<br>

1. Preparar o Arquivo de Entrada (CSV)
Primeiro, precisamos de uma lista. Vamos criar um arquivo simples onde cada linha contém o **Nome do Grupo** e uma **Descrição**, separados por um ponto e vírgula `;`.
 
   ```bash
   vim lista_usuarios.csv
   ```

   <br>
   
   Cole o conteúdo abaixo (exemplo):

   ```csv
   ana.souza;Mudar123!;Financeiro
   bruno.alves;Mudar123!;Financeiro
   carla.dias;Mudar123!;Financeiro
   julia.pereira;Mudar123!;Financeiro
   daniel.rocha;Mudar123!;RH
   elisa.martins;Mudar123!;RH
   fabio.costa;Mudar123!;RH
   igor.santos;Mudar123!;RH
   gabriel.lima;Mudar123!;TI
   helena.silva;Mudar123!;TI
   luis.divino;Mudar123!;Diretoria
   ```
   <br>


2. O Script de Automação
   
   Crie o arquivo do script:

   ```bash
   vim criar_usuarios.sh
   ```

   <br>
   
   Cole o seguinte código:

   ```bash
   #!/bin/bash

   # =======================================================
   # SCRIPT: CRIAR USUÁRIOS E ADICIONAR A GRUPOS (SAMBA4)
   # Autor: Parceiro de Programacao
   # =======================================================

   ARQUIVO_CSV="lista_usuarios.csv"
   
   # Verifica se o arquivo existe
   if [ ! -f "$ARQUIVO_CSV" ]; then
       echo "Erro: O arquivo $ARQUIVO_CSV não foi encontrado."
       exit 1
   fi
   
   echo "--- Iniciando Importação de Usuários ---"
   
   # Lê o arquivo linha por linha
   # IFS=';' separa os campos pelo ponto e vírgula
   while IFS=';' read -r usuario senha grupo || [ -n "$usuario" ]; do
   
       # Pula linhas vazias
       if [ -z "$usuario" ]; then
           continue
       fi
   
       echo "Processando: $usuario..."
   
       # 1. Cria o usuário
       # --must-change-at-next-login: Força a troca de senha
       # --use-username-as-cn: Usa o nome de usuário como Nome Comum
       samba-tool user create "$usuario" "$senha" \
           --must-change-at-next-login \
           --use-username-as-cn
   
       # Verifica se a criação funcionou (código 0)
       if [ $? -eq 0 ]; then
           echo " > Usuário '$usuario' criado com sucesso."
           
           # 2. Adiciona ao Grupo
           # Só tenta adicionar se o grupo foi informado na planilha
           if [ -n "$grupo" ]; then
               samba-tool group addmembers "$grupo" "$usuario"
               
               if [ $? -eq 0 ]; then
                    echo " > Adicionado ao grupo '$grupo'."
               else
                    echo " ! AVISO: Falha ao adicionar ao grupo '$grupo'. O grupo existe?"
               fi
           fi
       else
           echo " ! ERRO: Falha ao criar usuário '$usuario'. (Talvez já exista?)"
       fi
       
       echo "-------------------------------------------"
   
   done < "$ARQUIVO_CSV"
   
   echo "Importação finalizada."
   ```

   <br>

3. Executando o Script

   Agora vamos dar permissão de execução ao script:
   
   ```bash
   chmod +x criar_usuarios.sh
   ```

   <br>
   
   Execute o script:

   ```bash
   bash criar_usuarios.sh
   ```
   <br>

4. Liste os usuários

   Liste os usuários para verificar ser foram criados corretamente:

   ```bash
   samba-tool user list
   ```
   <br>

5. Delete um usuário

   ```bash
   samba-tool user nome.usuario
   ```
---
